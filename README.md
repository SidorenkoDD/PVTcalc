...
> Проект находится в активной разработке. Flash, эксперименты, сохранение
> workspace и GUI уже образуют рабочую вертикаль. Критическая точка, единый
> Psat-контракт, внешний PVTSim baseline и релизная упаковка ещё не закрыты.
> Проверенный срез на 2026-07-24 описан в
> [`docs/AUDIT_2026-07-24.md`](docs/AUDIT_2026-07-24.md).
...
Статический анализ текущего CI-gate:
...
Ruff для `gui tests` и mypy для `gui`. Полный Ruff для `calc_core` пока не
является gate и на текущем срезе находит 66 замечаний; это открытый технический
долг, а не зелёная проверка всего репозитория. Численные regression-тесты
фиксируют внутренний результат движка и не считаются внешней верификацией.
Официальная сверка с PVTSim для KRSNLN будет добавлена после подготовки чистых
input/output fixtures с настройками симулятора.
...
- константы и допуски в Settings доступны только для чтения: `EngineConfig`
  ...
  не подключён;
- полный lint `calc_core` пока не включён в CI и содержит legacy-замечания;
...
- [`docs/AUDIT_2026-07-24.md`](docs/AUDIT_2026-07-24.md) — фактический срез,
  проверочные команды, границы ревизии и приоритеты после неё;
