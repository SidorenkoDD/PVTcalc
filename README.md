...
`gui_session.json` хранит состояние интерфейса, открытые вкладки и текущие
снимки результатов. По принятому плану результаты должны стать постоянной
частью проекта в общей базе, но миграция схемы ещё не реализована. Пока не
используйте session-файл как единственный архив инженерных результатов.
...
- Cancel скрывает результат завершившейся задачи, но ещё не останавливает сам
  численный солвер;
- константы и допуски в Settings доступны только для чтения до появления
  реального `EngineConfig`;
