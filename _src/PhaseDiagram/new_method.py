...
class SaturationPressure:
    ...
    def _find_S(self, p):
        ...
        if phase_stability_object.S_l > 1:
            ...
            if phase_stability_object.S_l > phase_stability_object.S_v:
                print('case1')
                k_values = np.array(list(phase_stability_object.k_values_liquid.values()))
                ...
                print('case2')
                k_values = np.array(list(phase_stability_object.k_values_vapour.values()))
                fugacities_mix = phase_stability_object._mixture_fugacities_arr
    ...
    def sp_process(self, lambd=1):
        ...
        # Подавляем предупреждения о делении на 0 или log(<=0), если в данных возможны такие случаи
        with np.errstate(divide='ignore', invalid='ignore'):
            self.Sum = np.sum(np.log(r_sp) / np.log(ratio))
            
        self.Ykz = np.sum(ratio)

        print(f"p={self.p_i:.2f}, sum_y_sp={self.sum_y_sp:.6f}, Ykz={self.Ykz:.6f}")
        # 5. Проверка сходимости
        if (np.abs(1.0 - self.sum_y_sp) < 1e-5) or (self.Ykz ** 2 < 1e-6):
    ...
    def sp_convergence_loop(self):
        self.sp_process()
        if self.p_max_bar_sat - self.p_min_bar_sat < math.pow(10, -12):
            return None
        
        while not (abs(1 - self.sum_y_sp) < math.pow(10, -5) or math.pow(self.Ykz, 2) < math.pow(10, -6)):
            self.sp_process()
            if self.p_max_bar_sat - self.p_min_bar_sat < math.pow(10, -12):
                return None
