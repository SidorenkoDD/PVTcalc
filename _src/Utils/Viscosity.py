    ...
    * calculate : main method to calculate viscosity
    ...
    def _calculate_epsilon_parameter(self):
        ...
        for component in list(self._mole_fractions.keys()):
            xi = self._mole_fractions[component]
            Tci_K = self._composition_data['critical_temperature'][component]
            Mi = self._composition_data['molar_mass'][component]
            Pci_atm = 0.9869233 * self._composition_data['critical_pressure'][component]

            xi_pci_sum += xi * Pci_atm
            xi_tci_sum += xi * Tci_K
            xi_mi_sum += xi * Mi
    ...
    def _calculate_mu0_mod(self):
        ...
        for component in list(self._mole_fractions.keys()):
            ...
            Mi = self._composition_data['molar_mass'][component]
            Pci_atm = 0.9869233 * self._composition_data['critical_pressure'][component]
            Tri = self._temperature / Tci_K

            epsilon_i = math.pow(Tci_K, 1 / 6) / (math.pow(Mi, 1 / 2) *  math.pow(Pci_atm, 2 / 3))

            if Tri < 1.5:
                mu_i = 34 * math.pow(10, -5) * (1 / epsilon_i) * math.pow(Tri, 0.94)
            else:
                mu_i = 17.78 * math.pow(10, -5) * (1 / epsilon_i) * math.pow(4.58 * Tri - 1.67, 5 / 8)

            sum_zi_mui_mi += xi * mu_i * math.pow(Mi, 1 / 2)
            sum_zi_mi += xi * math.pow(Mi, 1 / 2)

        return sum_zi_mui_mi / sum_zi_mi
