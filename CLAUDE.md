...
Самописный движок компонентного PVT-моделирования флюидов (нефть/газ/конденсат) на Python. Живой расчётный цикл: описание состава флюида → расчёт равновесных свойств компонентов (в т.ч. псевдокомпонентов C7+) → уравнение состояния (EOS) → тест термодинамической стабильности → двухфазный флэш → стандартные PVT-эксперименты (CCE, DLE, сепарация) → построение фазовой диаграммы (давление насыщения, давление конденсации, критическая точка, фазовая огибающая). CVD остаётся изолированной замороженной заготовкой и не входит в продуктовый путь.
...
Автор — практикующий инженер-разработчик модели, ведёт разработку в одиночку (иногда с правками коллеги "Дениса" — см. файлы `*_den_v.py`). По словам автора, текущая версия расчётов сходится с результатами коммерческого симулятора **PVTSim**. На срезе 2026-07-24 явного регрессионного теста, фиксирующего это сравнение в репозитории, пока нет; найденные Excel-отчёты не заменяют clean input/output fixture (см. [Known Issues](#known-issues--bugs) и [`docs/AUDIT_2026-07-24.md`](docs/AUDIT_2026-07-24.md)).

**Актуальный проверочный срез 2026-07-24:** `pytest` — 305 passed, CI-scoped
Ruff (`gui tests`) и `mypy gui` проходят. Полный Ruff по `calc_core gui tests`
даёт 66 замечаний и намеренно вынесен в отдельный backlog, чтобы не смешивать
стилевую чистку с изменениями численных алгоритмов.
...
| Persist-слой | `calc_core/Utils/ModelStore.py` (схема/валидация/lock/backup), `Export.py` (`ModelJSONDB`), `Results.py` (`ResultStore`) | — (`ResultsViewer.py` удалён) |
...
- `calc_core/Utils/Import.py` (`DBModelImport`) — пустая ранняя заготовка, нигде не вызывается. Задуманная функция чтения с 2026-07-18 реализована единым `ModelStore` и подключена к `Composition.from_db`/GUI; класс остаётся кандидатом на отдельное удаление, не вводить его параллельно в живой API.
...
**2026-07-19**: ноутбучные реализации критической точки/фазовой диаграммы удалены **по решению автора** — `CriticalPointCalculator`/`CriticalPointCalculatorParrallel` (сеточный поиск по метрике близости S→1) и `PhaseDiagramCalculator` в стиле VBA-бисекции (секция "#### From VBA") больше не существуют ни в `test_notebook.ipynb`, ни где-либо ещё. Прежнее указание «уточнить у пользователя, прежде чем их трогать» снято — уточнять больше нечего. Единственная живая реализация критической точки — `calc_core/PhaseEnvelope/CriticalPoint.py::CriticalPointCalculator`; **её результат сейчас недостоверен на многокомпонентных составах** (`gap` ≈ 772 бар, см. предупреждение в докстринге `PhaseEnvelopeFacade.critical_point` и `docs/BACKLOG.md`).

Тем же заходом в ноутбуке починены мёртвые импорты (`_src.*` → `calc_core.*`, `calc_core.PhaseDiagram.*` → `calc_core.PhaseEnvelope.*`, `Export2`/`CCE2` → `Export`/`CCE`, `PhaseIndentificator` → `PhaseIdentificator`). Правки механические, **прогоном ноутбука не проверены** — CI его не исполняет.
...
- `models.json` (корень репозитория) — персистентный "снэпшот"-стор: полный состав + рассчитанные свойства + история результатов флэша для именованных моделей (сейчас — `KRSNL_PVTSIM`/"KRASNOLEN_PVTSIM_COMPARISON" и `PRRZLM_MDT_TEST`). Все живые чтения/записи проходят через `calc_core/Utils/ModelStore.py`: per-record `schema_version=1`, строгая валидация/legacy migration в памяти, атомарная замена под lock, revision check и backup-ротация `.bak`…`.bak.2`. Простое чтение старого файла его не переписывает. `ModelJSONDB` и `Composition.from_db(path)` являются адаптерами над этим слоем. **Важно**: это не независимый эталонный датасет PVTSim — сюда пишутся результаты расчётов самого движка PVTcalc, а название `KRSNL_PVTSIM` означает лишь то, что этот конкретный состав когда-то сверялся с PVTSim во внешнем инструменте, но само численное сравнение в репозитории не зафиксировано.
...
- Constants в GUI намеренно read-only; редактирование появится только вместе с явным `EngineConfig`, передаваемым расчётным модулям.
...
2026-07-19 (сессия): входной `gui_session.json` и workspace codec нормализуют повреждённые структуры, пропускают отдельные невалидные узлы и не позволяют мусорным sequence/размерам окна уронить запуск GUI. Валидные части сессии сохраняются; детали — `docs/GUI.md`, СТАТУС-11.
