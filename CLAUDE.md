...
| Persist-слой | `_src/Utils/Export2.py` (`ModelJSONDB`), `Results2.py` (`ResultStore`) | — (`Export.py`/`Results.py`/`ResultsViewer.py`/`BaseClasses.py` удалены) |
...
- Статические свойства чистых компонентов (молярная масса, Tb, критические параметры, ацентрический фактор, shift-параметр, BIP, Kw) — в `_src/Utils/DB_V2.json`, читаются через `_src/Utils/JsonDBReader.py` (он же умеет искать файл по нескольким кандидатным путям — cwd, родительские директории, `$DB_PATH`, каталог `sys.argv[0]`; это позволяет запускать код из разных рабочих директорий, но одновременно скрывает ошибку, если файл не найден там, где ожидалось).
